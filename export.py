...
def start_exporting(count):
    logging.debug(str.format("start_exporting - count={}, len(exporting_agents)={} (MAX_EXPORTS={}), len(agents_queue)={}",
                             count, len(exporting_agents), MAX_EXPORTS, len(agents_queue)))
    while len(exporting_agents) < MAX_EXPORTS and len(agents_queue) > 0:
        ...
        if start_time >= final_end_time:
            logging.info(str.format("Nothing to export for agent {} since registeredTime={} and lastHealthPingTime={}",
                                    agent['agentId'], reg_time, last_health_time))
            continue

        logging.info(str.format("Export for agent {} will start at {} (registeredTime={}) and end at {} (lastHealthPingTime={})",
                                agent['agentId'], start_time, reg_time, final_end_time, last_health_time))
        try:
            ...
            exporting_agents[agent['agentId']] = [
                start_time, final_end_time, response['exportId']]
        except Exception as e:
            if (type(e).__name__ == "OperationNotPermittedException"):
                #logging.info(str.format("e = {}", e))
                #last_word = e.message.split()[-1]
                last_word = str(e).split()[-1]
                if last_word == "another.":
                    ...
                    logging.info(str.format(
                        "start_exporting - Maximum number of concurrent exports exceeded. Requeuing agent {} and waiting...", agent['agentId']))
                    time.sleep(8)
                else:
                    ...
                    #logging.info(str.format(
                    #   "start_exporting - OperationNotPermittedException for agent {}: {}", agent['agentId'], e.message))
                    logging.info(str.format(
                        "start_exporting - OperationNotPermittedException for agent {}: {}", agent['agentId'], e))
...
def poll_exports(dir_name):
    ...
    for agent_id in exporting_agents:
        logging.info("Trying to export data for " + agent_id +
                     " from " + str(exporting_agents[agent_id][0]))
        export_response = client.describe_export_tasks(exportIds=[exporting_agents[agent_id][2]], filters=[
                                                       {'name': 'agentIds', 'values': [agent_id], 'condition': 'EQUALS'}])
        ...
        if exports_info['exportStatus'] in ["SUCCEEDED", "FAILED"]:
            ...
            if exports_info['exportStatus'] == "SUCCEEDED":
                (actual_start, actual_end) = extract_exports(
                    exports_info, agent_id, dir_name)
            else:
                logging.info(str.format(
                    "exportId {}: {} - {}", exports_info['exportId'], exports_info['exportStatus'], exports_info['statusMessage']))
            ...
            # Otherwise, go to next export
                ...
                next_end_time = min(
                    next_start_time + datetime.timedelta(days=3), exporting_agents[agent_id][1])
                logging.info(str.format(
                    "Next export for agent {} will continue at {} and end at {}", agent_id, next_start_time, next_end_time))
                try:
                ...
                # If successful export already exists, use exportId of existing export
                    ...
                    if (type(e).__name__ == "OperationNotPermittedException"):
                        ...
                        if (last_word == "another."):  # Too many concurrent exports
                            logging.info(
                                "poll_exports - Maximum number of concurrent exports exceeded. Waiting...")
                            time.sleep(8)
                        else:  # Export already exists
                            logging.info(str.format(
                                "poll_exports - OperationNotPermittedException for agent {}: {}", agent_id, e.message))
            ...
            logging.info(str.format("ERROR: Unknown status for exportId {}: {} - {}",
                                    exports_info['exportId'], exports_info['exportStatus'], exports_info['statusMessage']))
    for agent_id in done:
        del exporting_agents[agent_id]
    logging.debug(str.format(
        "Exiting poll_exports - {} agents were done exporting, {} still exporting", len(done), len(exporting_agents)))
    ...
    for num_retry in range(num_retries):
        try:
            ...
            logging.error(str.format(
                "download of {} failed on {}th retry with HTTP Error: {}", url, num_retry, e.code))
        except URLError as e:
            logging.error(str.format(
                "download of {} failed on {}th retry with URL Error: {}", url, num_retry, e.reason))
        else:
...
def extract_exports(exports_info, agent_id, dir_name):
    logging.debug(str.format("extracting {}, url={}",
                             exports_info['exportId'], exports_info['configurationsDownloadUrl']))
    ...
    with ZipFile(BytesIO(zipped)) as zip_ref:
        for name in zip_ref.namelist():
            ...
            if subdir == "results":
                ...
                if 'ActualEndTime' in d['ExportSummary']:
                    actual_start = get_time(
                        d['ExportSummary']['ActualStartTime'], True)
                    actual_end = get_time(
                        d['ExportSummary']['ActualEndTime'], True)
            ...
            target_dir = os.path.join(
                dir_name, "agentExports", agent_id, subdir)
            try:
                os.makedirs(target_dir)
            except OSError:  # already exists
                pass
            source = TextIOWrapper(zip_ref.open(name))
...
def parse_args():
    ...
    parser.add_argument("--end-time", help="The end timestamp until which exported data will be collected. Format: YYYY-MM-DDTHH:MM",
                        type=lambda d: datetime.datetime.strptime(d, '%Y-%m-%dT%H:%M'))
    parser.add_argument(
        "--filters", help="List of agentIds for which exported data will be collected.", nargs='+', type=str)
    parser.add_argument(
        "--log-file", help="File name where logs will be written, instead of the console", dest="log_file")
...
if __name__ == '__main__':
    ...
    if log_file:
        print(str.format(
            "Debug log file {} configured; this will be the last message to the console.", log_file))
    ...
    client = boto3.client('discovery', region_name=REGION)
    ...
    while moreAgents:
        response = client.describe_agents(
            maxResults=MAX_DESCRIBE_AGENTS, nextToken=nextToken)
        if filters:
            ...
            agents_queue += [agent for agent in response['agentsInfo']
                             if "connector" not in agent['agentType'].lower()]
    ...
    while len(agents_queue) > 0 or len(exporting_agents) > 0:
        logging.debug(str.format("Main export loop - {} agents in export queue, {} agents currently waiting to export, count={}",
                                 len(agents_queue), len(exporting_agents), count))
